...
window.gc.handlePhoto = function(e) {
  ...
  reader.onload = async ev => {
    ...
    // Mostrar indicador de análisis IA
    const uploadArea = document.querySelector('.upload-area');
    if (uploadArea) {
      uploadArea.innerHTML = '<span style="font-size:13px;color:#185FA5">🤖 Analizando factura con IA...</span>';
      uploadArea.style.borderColor = '#185FA5';
    }
    ...
    try {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              {
                type: 'text',
                text: 'Analiza esta factura o recibo de compra. Extrae SOLO estos datos en formato JSON sin ningún texto adicional ni backticks: {"valor": número sin puntos ni comas solo dígitos, "descripcion": "descripción breve del producto o servicio máximo 80 caracteres"}. Si no puedes leer algún dato usa null.'
              }
            ]
          }]
        })
      ...
      if (parsed.valor && parsed.valor > 0) {
        const valorEl = document.getElementById('wValor');
        if (valorEl) { valorEl.value = parsed.valor; valorEl.style.border = '2px solid #1D9E75'; }
      }
      if (parsed.descripcion) {
        const descEl = document.getElementById('wDesc');
        if (descEl) { descEl.value = parsed.descripcion; descEl.style.border = '2px solid #1D9E75'; }
      }

      // Actualizar indicador
      if (uploadArea) {
        const camposLeidos = [parsed.valor, parsed.descripcion].filter(Boolean).length;
        uploadArea.innerHTML = camposLeidos > 0
          ? `<span style="font-size:13px;color:#1D9E75">✅ IA completó ${camposLeidos} campo(s) automáticamente — verifica los datos</span>`
          : '<span style="font-size:13px;color:#BA7517">⚠️ No se pudo leer la factura — completa los campos manualmente</span>';
      }

    } catch (err) {
      console.error('Error IA:', err);
      const uploadArea = document.querySelector('.upload-area');
      if (uploadArea) uploadArea.innerHTML = '<span style="font-size:13px;color:#BA7517">⚠️ No se pudo analizar — completa los campos manualmente</span>';
    }
  };
